...
class LoudQuery(BaseQuery):
    ...
    def get_by_cycle(self, user_lat, user_lon):
        ...
        return self.filter(sql.or_(Loud.is_admin==True, abs(earth_r*acos(sin(user_lat)*sin(Loud.lat)*cos(user_lon-Loud.lon)+cos(user_lat)*cos(Loud.lat))*pi()/180)<distance))
